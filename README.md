...
encoder 行靠**横向拖**改值，而参数抽屉要能**纵向滚**。这两件事在触屏上打架，早先两处都做错了：

1. `.enc-native` 是一个 `opacity:0` 铺满整行的原生 `<input type="range">`（留着是为了键盘和读屏）。
   触屏事件先落在它身上，而原生 range 的默认行为是**点哪儿就跳到哪儿的值**——
   于是在抽屉里随手一点（比如想停住惯性滚动），那一行的参数就变了。
   实测「灯数量」单击行中间 48→49，点行右侧直接 49→**83**。
2. 行的 `pointerdown` 一进来就 `dragging = true`，竖滑只要带一点横向分量就开始改值。
   浏览器要过一会儿才判定成滚动并发 `pointercancel`，但值早就动了。
   实测横 40px / 竖 200px 的斜滑：抽屉滚了，值也被改了。

修法两条：原生 range 加 `pointer-events: none` 交出指针（不影响 Tab 聚焦和方向键），
指针输入统一由行接管；触屏先进 `pending`，**等手势自己证明是横向的**才开始改值——
一旦纵向先越过 10px 阈值就永久判给滚动，本次手势再怎么横move 也不改值。
宁可少改一次，也不要在滑列表时误改参数。鼠标没有"拖着滚动"这回事，不走这套判定。

验收：24 次点击 + 8 次带抖动竖滑遍历全部可见滑块行，**零误改**；
同时横拖改值、斜横拖、鼠标拖、方向键、Shift 精调、双击回默认全部照常。
